...
| Provide usage analytics    | The walkthrough starts the real in-memory dashboard/analytics service and queries `/v1/usage` by customer, job, provider, and outcome after persisting hourly, daily, and per-customer rollups.                                                                     | The private production dashboard and `/v1/usage`, `/v1/usage/reconciliations`, `/v1/usage/events`, and `/v1/capacity` endpoints expose durable usage, cost, health, capacity, and policy evidence.                                                |
