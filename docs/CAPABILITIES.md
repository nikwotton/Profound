...
The production deployment does not replace the demo's routing implementation. It runs the same contracts and algorithms with production transports, persistence, evidence, and operational boundaries.
...
| -------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Track all proxy requests   | Every upstream attempt traverses the real data plane and is written to the in-memory usage ledger with operation, route, grant, customer, provider, protocol, outcome, timing, retry, byte, and pricing context. The walkthrough prints sanitized requests and responses. | The same idempotent attempt records are stored durably in DynamoDB across tasks and deployments. Active tunnels are tracked and heartbeated so long-lived work and connection-seconds remain observable.                                                |
| Collect meaningful metrics | The routing engine collects establishment outcomes, latency evidence, bytes, retry/failover context, selected candidate score, capacity pressure, and provider health. Demo state is ephemeral and OTLP export is deliberately disabled.                                  | Application services emit unsampled OTLP logs, traces, and metrics through task-local ADOT collectors to scoped Axiom datasets. High-cardinality routing evidence stays in restricted logs/traces and the usage ledger rather than metric labels.       |
| Enable cost attribution    | Usage records carry customer, principal/grant, route, provider, historical pricing version, transferred bytes, and connection timing. This proves that traffic can be attributed, but `pnpm demo` does not run periodic accounting or reconcile a provider invoice.       | The usage-accounting service produces hour/day/week/month rollups, estimates Bright Data per-GiB cost, allocates Proxidize device cost by connection-seconds, records `Unallocated` capacity, and reconciles estimates with configured provider totals. |
| Provide usage analytics    | The query, grouping, rollup, capacity, and reconciliation implementations are present, while the scripted demo focuses on request behavior and does not start the dashboard service.                                                                                      | The private company-facing dashboard and `/api/usage`, `/api/usage/reconciliations`, `/api/usage/events`, and `/api/capacity` endpoints expose usage, cost, health, capacity, and policy evidence.                                                      |

The practical boundary is: **the demo proves collection and attribution at request time; production services make that evidence durable, aggregated, searchable, and operationally useful.**
...
| Enable dynamic provider switching          | The routing engine evaluates compatible providers for each new connection. The walkthrough visibly routes residential and mobile profiles through different adapters without changing the client protocol.                                                                                                                                       | Live health, inventory, capacity circuits, and recent performance evidence continuously affect eligibility and ranking. A profile's explicit provider override remains the only caller-visible provider constraint.                                                                                    |
| Support load balancing                     | The same versioned scoring algorithm ranks eligible providers and individual Proxidize slots using reliability, nonlinear headroom, performance, cost efficiency, and stability. Local slot claims prevent the demo from treating a device as infinitely available.                                                                              | Slot load and candidate circuits are shared durably across gateway tasks. Atomic claims, liveness expiry, soft-capacity evidence, and Fargate scaling keep concurrent instances coordinated.                                                                                                           |
...
| Requirement                                | Demo-mode v0                                                                                                                                                                                                         | Productionization already in the repository                                                                                                                                                                                                                                                                                               |
| ------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dynamic scaling of mobile proxy allocation | The core engine shares available mobile slots, atomically counts active connections, scores headroom, and treats soft saturation as routing evidence. In-memory state makes this reviewable without renting devices. | Durable slot inventory and active-load claims coordinate every data-plane task. Capacity policies calculate recommended slot counts and estimated monthly cost impact. **Buying, assigning, or removing Proxidize slots remains an explicit operator action in v0; the repository does not autonomously change the vendor subscription.** |
| Usage-based resource management            | The router uses connection load, observed throughput, prioritized-data forecasts, health, and capacity pressure when scoring candidates and recording evidence.                                                      | The accounting worker combines usage records with provisioned capacity to report current, peak, percentile, and time-weighted utilization; it emits idempotent capacity-pressure evidence and planning recommendations. ECS task count scales separately from paid mobile-slot inventory.                                                 |
| Cost optimization strategies               | Cost efficiency is one component of the versioned provider/candidate score, after hard compatibility and health gates. The local providers make the decision deterministic and free to exercise.                     | Historical provider prices, per-GiB estimates, device connection-seconds, reconciled totals, `Unallocated` capacity, capacity headroom, and recommendation cost deltas support routing and procurement decisions without allowing cost to bypass geography, safety, or health.                                                            |
...
| Residential rotation, mobile affinity, geographic/carrier matching, scoring, and health-aware selection | DynamoDB durability, point-in-time recovery, multi-task coordination, and ECS replacement |
| Per-attempt usage capture and sanitized request/response output                                         | ADOT/Axiom export, retention, dashboards, reconciliation feeds, and notification delivery |
