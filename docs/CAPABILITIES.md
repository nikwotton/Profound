...
| Collect meaningful metrics | The routing engine collects establishment outcomes, latency evidence, bytes, retry/failover context, selected candidate load, shadow roadmap scores, capacity pressure, and provider health. Demo state is ephemeral and OTLP export is deliberately disabled.            | Application services emit unsampled OTLP logs, traces, and metrics through task-local ADOT collectors to scoped Axiom datasets. High-cardinality routing evidence stays in restricted logs/traces and the usage ledger rather than metric labels.       |
