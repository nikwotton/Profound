...
import { createIntegrationTargets } from "./aws-integration-targets.js";
import { createOperationsServices } from "./aws-operations-services.js";
import { v0Policy } from "./aws-policy.js";
import { containerHttpHealth } from "./aws-service-config.js";
import { containerSecret } from "./aws-secrets.js";
import { createStateResources } from "./aws-state.js";
import { createTelemetryCollectorServices } from "./aws-telemetry-services.js";
  ...
  async run() {
    ...
    const applicationImage =
      ...
      ({
        ...
        args: { INCLUDE_DEV_TOOLS: production ? "false" : "true" },
      } as const);
    const deploymentId = process.env.RELEASE_SHA?.trim() || `sst-${$app.stage}`;
    const partition = aws.getPartitionOutput({}).partition;

    const { routeState, deploymentNotifications, deploymentCoordinator } = createStateResources({
      production,
    ...
    function otelEnvironment(
      ...
      return {
        ...
        OTEL_RESOURCE_ATTRIBUTES: `service.version=${deploymentId},deployment.environment.name=${$app.stage},cloud.provider=aws,cloud.platform=${cloudPlatform}`,
      };
    }
    ...
    const { telemetryCollector, canaryTelemetryCollector } = createTelemetryCollectorServices({
      cluster,
      canaryCluster,
      production,
      adotImage,
      ...
      containers: [
        {
          ...
          environment: {
            ...
            BLOCKED_TARGET_HOSTNAMES: v0Policy.routing.blockedTargetHostnames,
            ...
            STREAM_BUFFER_BYTES: v0Policy.routing.streamBufferBytes,
            MAX_HEADER_BYTES: v0Policy.routing.maxHeaderBytes,
    ...
    const { integrationTargetState, integrationTargetApi, integrationTargetRoute, integrationTransportTarget } = createIntegrationTargets({
      production,
      cloudTestStage,
      canaryCluster,
      applicationImage,
    ...
    const { healthAggregator, status, usageAccounting, notification } = createOperationsServices({
      cluster,
      production,
      routeState,
      applicationImage,
