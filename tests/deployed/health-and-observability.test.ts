...
deployedTest("deployed health aggregator and status application expose durable capability state and freshness", async () => {
  ...
  assert.ok(aggregatorBody.snapshot.capabilities.every(({ status }) =>
    status === "operational" || status === "degraded" || status === "unavailable"));
...
deployedTest("deployed signed public canary works directly and through the normal proxy path without replay", async (t) => {
  ...
  assert.equal((await fetch(challengeUrl, {
    method: "POST",
...
deployedTest("deployed passive traffic reaches the health aggregator through the product telemetry collector", async (t) => {
  ...
  const beforeValidation = before.capabilities.find(({ capability }) => capability === "unauthenticated_traffic")
    ?.endToEndValidatedAt;
  ...
  const updated = await waitFor("collector-fanned passive health", async () => {
    ...
    const validation = snapshot.capabilities.find(({ capability }) => capability === "unauthenticated_traffic")
      ?.endToEndValidatedAt;
...
deployedTest("deployed OTLP logs, metrics, traces, and canary security logs arrive in Axiom without sensitive payloads", async (t) => {
  ...
  const proxyLogs = await waitFor("proxy OTLP logs in Axiom", async () => {
  ...
  const securityLogs = await waitFor("canary security OTLP logs in Axiom", async () => {
    const logs = await axiomDatasetText(environment.metadata.telemetry.datasets.logs, startedAt);
  ...
  const metricText = await waitFor("OTLP metrics in Axiom", async () => {
  ...
  const spans = await waitFor("OTLP spans in Axiom", async () => {
