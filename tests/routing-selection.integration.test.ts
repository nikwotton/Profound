...
import { PublicAccessGrantSchema } from "../src/control-contract.js";
...
test("stateless mobile credentials share least-loaded proxy-slot capacity without affinity", async (t) => {
  ...
  const rotatedGrant = Schema.decodeUnknownSync(Schema.Struct({ grant: PublicAccessGrantSchema }))(await rotatedGrantResponse.json()).grant;
